...
import { TransportadorasServices } from "../../shared/services/api/transportadoras/TransportadorasServices";
...
export const DetalhesDeTransportadoras: React.FC = () => {
    ...
    useEffect(() => {
        if (id !== 'novo') { // se for diferente de novo
            ...
            TransportadorasServices.getById(Number(id))
            .then((result) => {
                setIsLoading(false);
                ...
                if (result instanceof Error) {
                    ...
                    setTransportadora(result.sufixo);
                    formRef.current?.setData(result);
                }
            });
        } else { 
            formRef.current?.setData({
                sufixo: '', documento: '', inscricao: '', ccm: '',
    ...
    const handleSave = (dados: IFormData) => {
        ...
        formValidationSchema.
            ...
            .then((dadosValidados) => {
                ...
                if (id === 'novo') {
                    TransportadorasServices.create(dadosValidados)
                    .then((result) => {
                        setIsLoading(false)
                        ...
                        if (result instanceof Error ) {
                            ...
                            if (isSaveAndClose()) {
                                ...
                                navigate(`/transportadoras/detalhesDetransportadoras/${result}`);   
                            }
                    ...
                    TransportadorasServices.updateById(Number(id), {id: Number(id), ...dadosValidados})
                    .then((result) => {
                        setIsLoading(false)
                        ...
                        if (result instanceof Error ) {
                            ...
                            setTransportadora(dados.sufixo);
                            if (isSaveAndClose()) {
                                navigate('/transportadoras');   
                            }
                        }
    ...
    const handleDelete = (id: number) => {
        ...
        TransportadorasServices.deleteById(id)
          .then(result => {
    ...
    return (
        <LayoutBase 
            titulo= {id === 'novo' ? 'Nova transportadora' : transportadora}
            barraDeFerramentas = {<FerramentasDeDetalhes 
                ...
                aoClicarBotaoNovo = {() => navigate('/transportadoras/detalhesDetransportadoras/novo')}
                aoClicarBotaoVoltar = {() => navigate('/transportadoras')}
        />}>
